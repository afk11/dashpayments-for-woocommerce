...
if ( ! class_exists( 'WC_Gateway_Bitcoin' ) ) :
    ...
    class WC_Gateway_Bitcoin extends WC_Payment_Gateway {
        private static $currency = 'Bitcoin';
        private static $currencyDesc = '<a href="https://www.bitcoin.org/" target="_blank">Bitcoin</a> is a peer-to-peer, decentralized digital currency that enables instant payments from anyone to anyone, anywhere in the world.';
        private static $currencyUrl = 'https://bitcoin.org';
        private static $iconPath = '/assets/images/dash-32x.png';
        private static $defaultInsight = 'https://insight.bitpay.com';
        private static $walletUrl = 'https://electrum.org';
        private static $walletTitle = 'Electrum Wallet';
        private static $currency_ticker_symbol = 'BTC';
        private static $currency_symbol = 'BTC';
        ...
        public function __construct() {
            ...
            $this->icon               = plugins_url(self::$iconPath, DP_PLUGIN_FILE);
            $this->has_fields         = false;
            $this->method_title       = __(self::$currency, 'dashpay-woocommerce');
            $this->method_description = __('Pay with '.self::$currency.' - ' . self::$currencyDesc, 'dashpay-woocommerce');
        ...
        protected function check_valid() {
            ...
            // Ensure xpub entered
            if ( ! $this->xpub_key ) {
                $msg = __("Pleace add a " . self::$currency . " BIP32 extended public key (Launch your Electrum-" . self::$currency . " wallet, Select Wallet->Master Public Keys)", 'dashpay-woocommerce');
                return $msg;
            }
        ...
        public function admin_notices() {
            ...
            if ( 0 !== count($missing) ) {
                $msg = sprintf(
                    __("<strong>" . self::$currency . " Payments for WooCommerce:</strong> Required extension(s) not loaded/enabled. Please enable '%s' PHP extension(s) on your WordPress server.", 'dashpay-woocommerce'),
                    join(', ', $missing)
                );
            ...
            // Check for xpub key
            if ( ! $this->xpub_key ) {
                self::_admin_error( __("Pleace add a " . self::$currency . " BIP32 extended public key (Launch your Electrum-" . self::$currency . " wallet, Select Wallet->Master Public Keys)", 'dashpay-woocommerce') );
                return false;
            }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __( 'Enable ' . self::$currency . ' Payments', 'dashpay-woocommerce' ),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __( 'Pay with ' . self::$currency, 'dashpay-woocommerce' ),
                    // 'desc_tip'    => true,
                ),
                'xpub_key' => array(
                    'title' => __( self::$currency . " BIP32 Extended Public Key", 'dashpay-woocommerce' ),
                    'type' => 'textarea',
                    'default' => '',
                    'description' => __('Start <a href="' . self::$walletUrl . '" target="_blank">' . self::$walletTitle . '</a> and get Master Public Key value from:<br>Wallet -> Master Public Key<br>Copy/paste the string starting with "xpub" into this field.', 'dashpay-woocommerce'),
                    // 'desc_tip'    => true,
                ),
                'confirmations' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('Pay with ' . self::$currency, 'dashpay-woocommerce') . "</h3>\n";
            echo '<p>' . __('Enables direct ' . self::$currency . ' payments with WooCommerce. ' . self::$currencyDesc, 'dashpay-woocommerce') . "</p>\n";

            $error_message = $this->check_valid();

            if ( $error_message ) {
                $msg = sprintf(
                    esc_html__(self::$currency . ' payment gateway NOT operational. %s', 'dashpay-woocommerce'),
                    $error_message
                );
                self::_OLD_admin_error( $msg );
            }
            else {
                self::_OLD_admin_success(__(self::$currency . ' payment gateway is operational.','dashpay-woocommerce'));
            }
